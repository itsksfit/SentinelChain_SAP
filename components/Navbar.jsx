import { Menu, ShieldCheck, Bell, User, Info, AlertTriangle } from 'lucide-react';
...
export default function Navbar() {
  ...
  return (
    <header className="h-16 border-b border-white/10 flex items-center justify-between px-6 bg-[#0f1115]/80 backdrop-blur-md sticky top-0 z-40">
      <div className="flex items-center gap-4 lg:hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-white">SentinelChain</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 relative">
        <div ref={dropdownRef} className="relative">
          ...
          {showNotifications && (
            <div className="absolute right-0 mt-3 w-80 bg-gray-900 border border-gray-700 rounded-xl shadow-2xl overflow-hidden z-50 animate-[fadeIn_0.2s_ease-out]">
              ...
              <div className="max-h-[300px] overflow-y-auto">
                {notifications.length === 0 ? (
                  ...
                  notifications.map((n) => {
                    const Icon = n.icon;
                    return (
                      <div key={n.id} className="p-3 border-b border-gray-800 hover:bg-white/5 transition-colors cursor-pointer flex gap-3">
                        <div className="mt-0.5"><Icon className={`w-4 h-4 ${n.color}`} /></div>
                        <div>
                          <p className="text-xs font-bold text-white mb-0.5">{n.title}</p>
                          <p className="text-[11px] text-gray-400">{n.text}</p>
        ...
        <button className="w-8 h-8 rounded-full bg-gray-800 border border-gray-700 flex items-center justify-center overflow-hidden hover:border-gray-500 transition-colors cursor-pointer">
