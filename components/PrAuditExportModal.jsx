...
export default function PrAuditExportModal({ isOpen, onClose, data = {} }) {
  ...
  const verifiedUrl = data.verifiedUrl && data.verifiedUrl !== '#' 
    ? data.verifiedUrl 
    : 'https://earthquake.usgs.gov/earthquakes/eventpage/us7000m8v5/executive';
  ...
  const alternatives = (data.matched_options && data.matched_options.length > 0)
    ...
    : (data.pin_compatible_alternatives && data.pin_compatible_alternatives.length > 0)
      ...
      : [
          { alt_part_id: "GD32F403RET6", vendor: "Mouser Electronics", lead_time_days: 3, unit_price: 4.28, final_price: 3.95, status: "✅ Within 15% Cap" },
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 overflow-y-auto">
      <style jsx global>{`
        @media print {
          ...
          #printable-audit-sheet, #printable-audit-sheet * {
      ...
      <div className="relative w-full max-w-3xl bg-white dark:bg-[#0f1115] border border-gray-200 dark:border-white/10 rounded-2xl shadow-2xl overflow-hidden my-8">
        
        {/* Header Control Bar (Hidden during print) */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-white/10 bg-gray-50/80 dark:bg-white/5 print:hidden">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handlePrint}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-bold transition-all shadow-md active:scale-95"
            >
            ...
            <div className="bg-gray-50 dark:bg-white/5 p-3.5 rounded-xl border border-gray-200 dark:border-white/10 space-y-2">
              ...
              <div className="flex items-center justify-between text-[11px] pt-1.5 border-t border-gray-200 dark:border-white/10">
                ...
                <a href={verifiedUrl} target="_blank" rel="noreferrer" className="text-indigo-600 dark:text-indigo-400 hover:underline flex items-center gap-1 font-mono text-[10.5px]">
                  {verifiedUrl.length > 55 ? verifiedUrl.substring(0, 55) + '...' : verifiedUrl} <ExternalLink className="w-2.5 h-2.5" />
                </a>
              </div>
            </div>
          </div>
              ...
              <table className="w-full text-left text-[11px]">
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-white/5 font-mono text-[11px]">
                  {alternatives.slice(0, 3).map((alt, idx) => {
                    ...
                    return (
                      <tr key={idx} className={isOptimal ? 'bg-indigo-50/50 dark:bg-indigo-950/10' : 'opacity-75'}>
