...
Ensure JSON is valid. NO markdown wrapping.`;
  ...
  try {
    ...
    let chatLog = [
      { from: "System", text: "Drafting RFQ for alternative parts..." },
      { from: "System", text: "Negotiation failed or timed out. Falling back to cached supplier agreements..." }
    ];
    let plan = [
      { vendor: "Primary Distributor", part: options[0]?.partNumber || "Unknown", quantity: 5000, days: 5, score: "Default Plan" }
    ];
    return res.status(200).json({ chatLog, rankedPlan: plan });
  }
}
