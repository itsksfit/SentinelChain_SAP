      ...
      <main className="flex-1 lg:ml-64 flex flex-col min-h-screen relative">
        ...
        <div className="p-6 max-w-6xl mx-auto w-full space-y-6">
          <div className="flex justify-between items-end mb-6">
            <div>
              <h1 className="text-3xl font-bold text-white tracking-tight">Recovery Plans</h1>
              <p className="text-gray-400 mt-1">AI-generated mitigation strategies and their execution status.</p>
            </div>
            <button 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {plans.map((plan, i) => (
              <div key={i} className={`glass-panel p-6 border ${plan.status === 'Executing' ? 'border-indigo-500/50 shadow-[0_0_15px_rgba(79,70,229,0.15)]' : 'border-white/10'}`}>
                ...
                <h3 className="text-lg font-bold text-white mb-1">{plan.trigger}</h3>
                <div className="flex items-center gap-2 text-sm text-gray-400 mb-4">
                  <span>Detected</span> <ArrowRight className="w-3 h-3" /> <span className="text-white font-medium">{plan.action}</span>
                </div>
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-[#0f1115] border border-gray-800 rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden animate-[fadeInUp_0.2s_ease-out]">
              <div className="p-6 border-b border-gray-800 flex justify-between items-center bg-gray-900/50">
                <h2 className="text-xl font-bold text-white flex items-center gap-2"><Zap className="w-5 h-5 text-indigo-400" /> AI Plan Generator</h2>
                <button onClick={() => setShowModal(false)} className="text-gray-500 hover:text-white">
                  <X className="w-5 h-5" />
                </button>
              </div>
              <div className="p-6">
                <p className="text-sm text-gray-400 mb-4">Describe a hypothetical disruption scenario or component shortage to generate a custom recovery plan.</p>
                <form onSubmit={handleGenerate}>
                  <div className="relative">
                    ...
                    <input 
                      ...
                      className="w-full bg-gray-900 border border-gray-700 rounded-xl py-3 pl-10 pr-4 text-white focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all placeholder-gray-600"
                    />
                  </div>
                  <div className="mt-6 flex justify-end gap-3">
                    ...
                    <button 
                      ...
                      className="bg-indigo-600 hover:bg-indigo-500 text-white px-6 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                    >
