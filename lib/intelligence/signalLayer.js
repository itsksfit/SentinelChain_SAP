 ...
 * - Tier 1: SEC EDGAR & Official Corporate IR Portals
...
function evaluateSeismicExposure(magnitude, distanceKm) {
  if (magnitude >= 6.0 && distanceKm <= 100) {
    return {
      riskLevel: "CRITICAL",
      description: "Severe ground motion exceeding 0.15g PGA. Stepper tool vibration interlocks, wafer micro-fractures, and cleanroom evacuations triggered.",
...
export function calculateEvidenceConfidence(sourceTiers = [], specificParam = null) {
  ...
  if (sourceTiers.includes("OFFICIAL_IR")) {
    return 80; // Foundry operational press statement
  }
  if (sourceTiers.includes("GSCPI_MACRO")) {
    return 74; // NY Fed macroeconomic stress index
  }
  return 68; // Media baseline
}
...
export async function fetchLiveSeismicSignals() {
  try {
    ...
    data.features.forEach(feature => {
      ...
      const eventUrl = feature.properties.url || "https://earthquake.usgs.gov/earthquakes/map/";
  ...
  return [
    {
      ...
      verifiedUrl: "https://nvidianews.nvidia.com",
